...
public class TestDtdParsing
{
	...
	public void testWithAttributes() throws Exception
	{
		...
		for (Container c : cont.children) {
			if (c.name.equals("a")) {
				assert c.attributes.size() == 1 : "Did not find exactly one attribute for 'a'";
			}
			if (c.name.equals("b")) {
				assert c.attributes.size() == 2 : "Did not find the expected two attributes for 'b'";
			}
			if (c.name.equals("c")) {
				assert c.attributes.size() == 0 : "Did not expect any attributes for 'c'";
			}
		}
	}
}
